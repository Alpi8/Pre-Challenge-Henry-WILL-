...
function crearClasePersona() {
  class Persona {
    ...
    addFriend(nombre, edad) {
      ...
      // Tu código aca:
      var agregar = {"nombre" : nombre, "edad" : edad}
      this.amigos.push(agregar)
    ...
    getFriends() {
      ...
      var nuevoArreglo = [];
          for (var i = 0; i < this.amigos.length; i++) { 
            nuevoArreglo.push(this.amigos[i].nombre)
            }
    ...
    for (var i = 0; i < this.amigos.length; i++) { 
      nuevoArreglo.push(this.amigos[i].edad)
      }

       for (let i = 0; i < nuevoArreglo.length; i++) {  
        suma = suma + nuevoArreglo[i]
        }
      promedio = suma / nuevoArreglo.length;
    return (promedio);
    }
  };
